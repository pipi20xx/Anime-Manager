                ...
                for idx, group in enumerate(data.get("groups", [])):
                    ...
                    if episodes:
                        ...
                        for tmdb_season in sorted(tmdb_seasons):
                            season_eps = [ep.get("episode_number", 0) for ep in episodes if ep.get("season_number") == tmdb_season]
                            if season_eps:
                                min_ep = min(season_eps)
                                max_ep = max(season_eps)
                                if min_ep == max_ep:
                                    ep_ranges.append(f"S{tmdb_season}E{min_ep}")
                                else:
