...
export const CUSTOMIZABLE_PAGES = {
  ...
  'explore-recommend': { label: '探索 → 播出时间表', description: '探索页面的播出时间表（Bangumi 每周番剧放送）', routeName: 'ExploreRecommend' },
  'explore-seasonal':  { label: '探索 → 季度番剧表', description: '探索页面的季度番剧表（按年份/季度展示番剧）',   routeName: 'ExploreSeasonal' },
  'explore-discover':  { label: '探索 → 探索索引',   description: '探索页面的 BGM 和 TMDB 发现页面',               routeName: 'ExploreDiscover' },
  'explore-search':    { label: '探索 → 聚合搜索',   description: '探索页面的聚合搜索',                           routeName: 'ExploreSearch' },
