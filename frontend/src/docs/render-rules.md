...
作用阶段：识别匹配完成后 **后处理**。
核心处理脚本：`backend/recognition/render/engine.py`
...
#### E. 基于 TMDB ID + 文件名关键词的季度修正
*   **规则**: `@?{[tmdbid=123456;type=tv;includes=2025]} => {[s=3]}`
*   **含义**: 当 TMDB ID 为 123456 且类型为 TV 且文件名包含 "2025" 时，强制设为 S3。
*   **场景**: 同一部剧的不同年份版本对应不同季度（如 2022 年版是 S1，2025 年版是 S3）。
*   **示例**:
    ```
    @?{[tmdbid=123456;type=tv;includes=2022]} => {[s=1]}
    @?{[tmdbid=123456;type=tv;includes=2023]} => {[s=2]}
    @?{[tmdbid=123456;type=tv;includes=2025]} => {[s=3]}
    ```
