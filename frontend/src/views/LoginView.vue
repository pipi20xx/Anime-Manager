...
const handleLogin = async () => {
  ...
  try {
    ...
    if (res.data.status === '2fa_required') {
      showOtp.value = true
      // 2FA 第一阶段通过，临时存 token 供 2FA 验证使用 (如果有需要的话，这里 backend api 设计可能不同)
      // 但在我们的 routers/auth.py 中，2fa_pending token 已经返回了。
      localStorage.setItem('apm_access_token', res.data.access_token)
